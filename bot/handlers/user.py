...
# Mavjud e'lonlar handleriga webapp tugmasini qo'shing
@user_router.message(F.text == "📋 Mavjud e'lonlar")
async def show_available_ads(message: Message, db: Database):
    """Mavjud e'lonlarni ko'rsatish va webapp tugmasini yuborish"""
    try:
        # Faol e'lonlar sonini olish
        stats = await db.get_stats()
        active_count = stats['listings']['active']
        
        await message.answer(
            f"🗺️ Hozirda {active_count} ta faol e'lon mavjud.\n\n"
            "Xaritada ko'rish uchun quyidagi tugmani bosing:",
            reply_markup=get_webapp_keyboard()
        )
    except Exception as e:
        logger.error(f"Show available ads error: {e}")
        await message.answer("❌ Xatolik yuz berdi. Qayta urinib ko'ring.")

# WebApp dan kelgan so'rovlarni qayta ishlash
@user_router.message(F.web_app_data)
async def handle_webapp_data(message: Message, db: Database, bot: Bot):
    """WebApp dan kelgan ma'lumotlarni qayta ishlash"""
    try:
        data = json.loads(message.web_app_data.data)
        action = data.get('action')
        
        if action == 'listing_details':
            listing_id = data.get('listing_id')
            listing = await db.get_listing(listing_id)
            
            if listing and listing.get('status') == 'active':
                # E'lon ma'lumotlarini foydalanuvchiga yuborish
                status_text = "✅ Faol" if listing.get('status') == 'active' else "❌ Nofaol"
                
                ad_text = (
                    f"🏠 <b>E'lon ma'lumotlari</b>\n\n"
                    f"📌 <b>Sarlavha:</b> {listing['title']}\n"
                    f"📝 <b>Tavsif:</b> {listing['description']}\n"
                    f"💰 <b>Narx:</b> {listing['price']} {listing.get('currency', 'UZS')}\n"
                    f"🏢 <b>Qavat:</b> {listing['floor']}\n"
                    f"🏠 <b>Xonalar:</b> {listing['rooms']}\n"
                ...
                if listing.get('photos') and len(listing['photos']) > 0:
                ...
                await message.answer("❌ Bu e'lon topilmadi yoki faol emas.")
...
async def confirm_delete_handler(callback: CallbackQuery, db: Database):
    """E'lonni o'chirishni tasdiqlash"""
    try:
        listing_id = int(callback.data.split("_")[2])
        is_admin = callback.from_user.id in ADMIN_IDS

        success = await db.delete_listing(listing_id, user_id=callback.from_user.id, is_admin=is_admin)
        ...
        if success:
            try:
                if callback.message.text:
                    ...
                    await callback.message.edit_caption("✅ E'lon muvaffaqiyatli o‘chirildi.")
                else:
                    await callback.message.delete()
            ...
            logger.info(f"✅ Listing {listing_id} deleted by user {callback.from_user.id}")
        else:
            await callback.answer("❌ E'lon topilmadi yoki sizga tegishli emas.", show_alert=True)

    except Exception as e:
        logger.error(f"Confirm delete handler error: {e}")
